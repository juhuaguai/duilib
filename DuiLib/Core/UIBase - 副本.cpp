...
LRESULT CALLBACK CWindowWnd::__ControlProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	else if (uMsg == WM_NCCALCSIZE)
	{
		...
		if ( wParam == TRUE)
		{
			...
			pRect0=&pParam->rgrc[0];
			pRect1=&pParam->rgrc[1];
			pRect2=&pParam->rgrc[2];
		}
		else
		{
			pRect0=(LPRECT)lParam;
		}
		...
		{	// 最大化时，计算当前显示器最适合宽高度
			...
			//如果有系统边框,则仍然保留windows的设计(正常情况下最大化每条边会超出屏幕各8像素)。若没有系统边框(窗口整个都是客户区),则处理为不超出屏幕
			bool bHave = false;
			if (pRect0 && pRect1 && pRect2)
			{
			...
			if (bHave)
			{
				pRect0->right = pRect0->left + rcWork.GetWidth()+16;
				pRect0->bottom = pRect0->top + rcWork.GetHeight()+16;
			}
			else
			{
				pRect0->right = pRect0->left + rcWork.GetWidth();
				pRect0->bottom = pRect0->top + rcWork.GetHeight();
			}
			return WVR_REDRAW;
		}
	}
