    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
		...
		//树控件XML解析
            ...
            case 8:
				...
				else if( _tcsicmp(pstrClass, DUI_CTR_APNGANIM) == 0 )         pControl = new CApngAnimUI;
                break;
            case 9:
				...
				else if( _tcsicmp(pstrClass, DUI_CTR_GIFBUTTON) == 0 )        pControl = new CGifButtonUI; 				
                break;
            case 10:
				...
				else if( _tcsicmp(pstrClass, DUI_CTR_APNGBUTTON) == 0 )       pControl = new CApngButtonUI;
                break;
			case 11:
				...
				else if (_tcsicmp(pstrClass, DUI_CTR_SCROLLLABEL) == 0)		  pControl = new CScrollLabelUI;	
				break;
            case 14:
