...
BOOL CQR_Encode::EncodeData(int nLevel, int nVersion, BOOL bAutoExtent, int nMaskingNo, LPCSTR lpsSource, int ncSource /* = 0 */,LPCWSTR lpDestFile /* = NULL */)
{
	int i, j;
	...
	if (lpDestFile && wcslen(lpDestFile)>0)
	{
		return SaveImgFile(lpDestFile);
	}
	return TRUE;
}

BOOL CQR_Encode::SaveImgFile(LPCWSTR lpDestFile)
{
	...
	bmi.bmiHeader.biWidth = nSymbleSize;
	bmi.bmiHeader.biHeight = nSymbleSize;
	bmi.bmiHeader.biPlanes = 1;
	bmi.bmiHeader.biBitCount = 32;
	bmi.bmiHeader.biCompression = BI_RGB;
	bmi.bmiHeader.biSizeImage = nSymbleSize * nSymbleSize * sizeof(DWORD);
	LPDWORD pDest = NULL;
	HBITMAP hBitmap = ::CreateDIBSection(hDc, &bmi, DIB_RGB_COLORS, (LPVOID*) &pDest, NULL, 0);
	if( !hBitmap ) return FALSE;
	
	HBITMAP hOldBitmap = (HBITMAP) ::SelectObject(hDc, hBitmap);
	PatBlt(hDc,0,0,nSymbleSize,nSymbleSize,WHITENESS);
	for (int i = 0; i < m_nSymbleSize; ++i)
	{
		for (int j = 0; j < m_nSymbleSize; ++j)
		{
			...
			{
				SetPixel(hDc,i + QR_MARGIN, j + QR_MARGIN, RGB(0, 0, 0));
			}
		}
	}
	DeleteDC(hDc);
	...
	CImage theImg;
	theImg.Attach(hBitmap);
	HRESULT hReslut = theImg.Save(lpDestFile,Gdiplus::ImageFormatPNG);
	theImg.Detach();
	if (hReslut != S_OK)
	{
		return FALSE;
